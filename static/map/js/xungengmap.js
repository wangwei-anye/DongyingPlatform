...
var Oeasyxungengmap = {
		...
		Iconstyle:function(feature) {
			return [new ol.style.Style({
				...
				image:new ol.style.Icon(({
				    ...
				    anchorYUnits: 'pixels',
					src:Oeasyxungengmap.Icon[feature.get("type")]
				})),
		...
		init:function(list){
			...
			Oeasyxungengmap.vectorLayer = new ol.layer.Vector({
				source : Oeasyxungengmap.vectorSource,
				style : Oeasyxungengmap.Iconstyle,
				zIndex : 1000
			});
			
			
			Oeasyxungengmap.map = new ol.Map({
				
		        target: document.getElementById("xungengmap"),
		        view: Oeasyxungengmap.view,
		      });
			Oeasyxungengmap.map.addLayer(Oeasyxungengmap.vectorLayer);
			Oeasyxungengmap.xqgeolayer = new ol.layer.Vector({
				source : Oeasyxungengmap.xqgeoSource
				
			});
			Oeasyxungengmap.map.addLayer(Oeasyxungengmap.xqgeolayer);
			
			Oeasyxungengmap.addcountylayer();
			
			setTimeout(function(){

				Oeasyxungengmap.addunitdata(list);
		...
		addunitdata:function(list){
			var city_code = Oeasyxungengmap.getUrlParam('city_code');
			var town_code = Oeasyxungengmap.getUrlParam('town_code');
			var township_code = Oeasyxungengmap.getUrlParam('township_code');
			var village_code = Oeasyxungengmap.getUrlParam('village_code');
			var dataparam = null;
			if(city_code == null){
				dataparam = {
			...
			$.ajax({
    			...
    			var data  = _dt.data;
				var projection = Oeasyxungengmap.map.getView().getProjection();
				if(data != null){
					if(data.length > 0){
    				for (var i = 0; i < data.length; i++) {
        				...
        				if (projection.getCode() != "EPSG:4326") {
        					var lnglat = Oeasyxungengmap.lonlat2mercator(lng, lat);
        				...
        				Oeasyxungengmap.heatmapSource.addFeature(iconFeature);
        			}
        			
        			//添加热力图
        			Oeasyxungengmap.heatmaplayer = new ol.layer.Heatmap({  
                        source: Oeasyxungengmap.heatmapSource,  
                        blur:60,
                        radius: 30,
                        opacity : 0.8
                    }); 
        			
        			Oeasyxungengmap.map.addLayer(Oeasyxungengmap.heatmaplayer);
    			   }else{
			...
			$.ajax({
				...
				if(features.length >0){
					for (var i = 0; i < features.length; i++) {
						...
						f.setStyle(Oeasyxungengmap.tongrenshiStyle(Oeasyxungengmap.dicolor));
						
					}
					Oeasyxungengmap.xqgeoSource.addFeatures(addNameFeatures);
			...
			$.ajax({
				...
				if(features.length >0){
					for (var i = 0; i < features.length; i++) {
						...
						f.set("adcode99", f.U.ADCODE99);
						f.setId(f.U.ADCODE99);
						f.setStyle(Oeasyxungengmap.allPCSStyle(Oeasyxungengmap.dicolor));
						//根据区域中心，设置中心点名称图标
						var point = null;
						...
						if(city_code ==undefined && town_code == undefined || city_code =="" && town_code == ""){
						    ...
						    Oeasyxungengmap.vectorSource.addFeature(iconFeature);
						}else if(city_code != '' && town_code == ""){
							addNameFeatures.push(f);
							point = new ol.geom.Point([ f.U.CENTROID_X, f.U.CENTROID_Y ]);
						    iconFeature = new ol.Feature({
							...
							//居中定位
					        var coordinates = f.U;
					        Oeasyxungengmap.map.setView(new ol.View({ 
						        projection: "EPSG:4326",
