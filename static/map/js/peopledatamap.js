...
$(document).ready(function(){
	...
	Oeasypeopledatamap.init();
});

var Oeasypeopledatamap = {
		map:null,
		...
		Iconstyle:function(feature) {
			return [new ol.style.Style({
				...
				image:new ol.style.Icon(({
				    ...
				    anchorYUnits: 'pixels',
					src:Oeasypeopledatamap.Icon[feature.get("type")]
				})),
	   ...
	   allPCSStyle:function(color){
		...
		init:function(list){
			...
			Oeasypeopledatamap.vectorLayer = new ol.layer.Vector({
				source : Oeasypeopledatamap.vectorSource,
				style : Oeasypeopledatamap.Iconstyle,
				zIndex : 1000
			});
			
			
			Oeasypeopledatamap.map = new ol.Map({
				
		        target: document.getElementById("peoplemap"),
		        view: Oeasypeopledatamap.view,
		      });
			Oeasypeopledatamap.map.addLayer(Oeasypeopledatamap.vectorLayer);
			Oeasypeopledatamap.xqgeolayer = new ol.layer.Vector({
				source : Oeasypeopledatamap.xqgeoSource
				
			});
			Oeasypeopledatamap.map.addLayer(Oeasypeopledatamap.xqgeolayer);
			
			Oeasypeopledatamap.addcountylayer();
			
			setTimeout(function(){

				Oeasypeopledatamap.addunitdata(list);
		...
		addunitdata:function(list){
			var city_code = Oeasypeopledatamap.getUrlParam('city_code');
			var town_code = Oeasypeopledatamap.getUrlParam('town_code');
			var township_code = Oeasypeopledatamap.getUrlParam('township_code');
			var village_code = Oeasypeopledatamap.getUrlParam('village_code');
			var dataparam = null;
			if(city_code == null){
				dataparam = {
			...
			$.ajax({
    			...
    			var data  = _dt.data;
    			var projection = Oeasypeopledatamap.map.getView().getProjection();
				if(data != null){
					if(data.length > 0){
    				 for (var i = 0; i < data.length; i++) {
        				...
        				if (projection.getCode() != "EPSG:4326") {
        					var lnglat = Oeasypeopledatamap.lonlat2mercator(lng, lat);
        				...
        				Oeasypeopledatamap.heatmapSource.addFeature(iconFeature);
        			 }
        			
        			//添加热力图
        			Oeasypeopledatamap.heatmaplayer = new ol.layer.Heatmap({  
                        source: Oeasypeopledatamap.heatmapSource,  
                        blur:60,
                        radius: 20,
                        opacity : 0.8
                     }); 
        			
        			Oeasypeopledatamap.map.addLayer(Oeasypeopledatamap.heatmaplayer);
    			    }else{
			...
			$.ajax({
				...
				if(features.length >0){
					for (var i = 0; i < features.length; i++) {
						...
						f.setStyle(Oeasypeopledatamap.tongrenshiStyle(Oeasypeopledatamap.dicolor));
						
					}
					Oeasypeopledatamap.xqgeoSource.addFeatures(addNameFeatures);
			...
			$.ajax({
				...
				if(features.length >0){
					...
					for (var i = 0; i < features.length; i++) {
						...
						f.set("adcode99", f.U.ADCODE99);
						f.setId(f.U.ADCODE99);
						f.setStyle(Oeasypeopledatamap.allPCSStyle(Oeasypeopledatamap.dicolor));
						//根据区域中心，设置中心点名称图标
						var point = null;
						...
						if(city_code ==undefined && town_code == undefined || city_code =="" && town_code == ""){
						    ...
						    Oeasypeopledatamap.vectorSource.addFeature(iconFeature);
						}else if(city_code != '' && town_code == ""){
							addNameFeatures.push(f);
							point = new ol.geom.Point([ f.U.CENTROID_X, f.U.CENTROID_Y ]);
						    iconFeature = new ol.Feature({
							...
							//居中定位
					        var coordinates = f.U;
					        Oeasypeopledatamap.map.setView(new ol.View({ 
						        projection: "EPSG:4326",
