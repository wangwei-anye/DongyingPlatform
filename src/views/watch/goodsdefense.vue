<template>
	<div class="goodsdefense">
		<TwoHeader title="物防数据"></TwoHeader>
		<div class="contentPage">
			<v-panel title="物防数据" class="v-panel" :width="1864">
				<div class="main-content">
					...
					<div class="table-detail" v-for="item, index in datalist">
	                  ...
	                  <div class="goods-tool">
	                    <Button v-if="goodsindex != index" class="app-button-table" @click="setGoodsDefense(index, item.slice(1))">
	                      <Icon type="edit" />
	                    </Button>
	                  </div>
	                  ...
	                  <div class="tool-button" v-if="goodsindex == index">
	                    <Button class="app-button-confirm" @click="submitGoods(item[0].unit_id)">确 认</Button>
  ...
  import { province_code, city_code } from '@/config'
	...
	export default {
      ...
      switch(level) {
		...
		methods: {
			...
			getGoodsList: function () {
				...
				axios.get('/safety/objectSecurityTotalSearch.do' + params).then(response => {
					if(response.data.success) {
      ...
      submitGoods: function (unit_id) {
        ...
        axios.post('/safety/updateObjectSecurityTotal.do?' + str).then(response => {
          if (response.data.success) {
            this.getGoodsList()
          }
          else {
