<template>
	<div class="techsetmodal">
		<div class="modal-content">
			<div class="modal-content-header">技防类目设置</div>
			<div class="modal-content-info">
				<div class="modal-content-subheader">当前技防类目</div>
...
<script>
  ...
  import { province_code, city_code } from '@/config'
    ...
    mounted: function () {
      ...
      this.getTechnicalDefenceCategorySearch()
    },
		methods: {
			// 获取技防类目
			getTechnicalDefenceCategorySearch () {
        axios.post('/safety/technicalDefenceCategorySearch.do', {
          province_code: this.currentProvince,
          city_code: this.currentCity
        }).then(response => {
          if(response.data.success) {
            this.categorylist = response.data.data.technicalNames
          }
          else {
            console.log('获取技防类目失败')
          }
        }).catch(err => {
          console.log('获取技防类目失败')
        })
        ...
        axios.get('/safety/technicalDefenceCategoryStandard.do?' + params).then(response => {
          if(response.data.success) {
            this.$emit('closeModal')
          }
          else {
            console.log('设置技防类目失败')
          }
        }).catch(err => {
          console.log('设置技防类目失败')
        })
