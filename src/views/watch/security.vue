<template>
	<div class="security">
		<div class="main-content">
			...
			<div class="table-detail" v-for="item in datalist">
		        ...
		        <Panel>
		        	...
		        	<b class="icon iconfont icon-font-baoan"></b>
		        	<label>专职保安队伍（{{ item[0].fullTimeCount }}人）</label>
		        	<div slot="content">
								<div class="addsecurity">
									<img src="../../assets/image/special/icon/tianjia.png" height="38" width="38" alt="" @click="addSecurity(item[0].unitId)">
								</div>
		...
		<SecurityEdit v-if="showedit" :idcard="idcard" :security="selectSecurity" @editsure="editsure" @editcancle="editcancle">
		</SecurityEdit>
		<SecurityDelete v-if="showdel" :security="selectSecurity" @delsure="delsure" @delcancle="delcancle"></SecurityDelete>
...
<script>
  ...
  import SecurityEdit from '@/views/watch/securityedit'
  import SecurityDelete from '@/views/watch/securitydelete'
          ...
          { 
            ...
            render: (h, params) => {
              return h('div', [
	                ...
	                on: { click: (e) => { this.updSecurity(params.row) } }
	              }),
								h('i', {
	                ...
	                on: { click: (e) => { this.delSecurity(params.row) } }
	              })
              ])
            }
          }
      ...
      switch(level) {
		...
		methods: {
			...
			getSecurityList: function () {
				axios.post('/safety/fullTimeSecurityTeamDetails.do', {
					province_code: this.currentProvince,
					city_code: this.currentCity,
			...
			editsure: function () {
				// 必填项
				if (!this.selectSecurity.name.trim()) {
					this.$Message.warning('姓名为必填项')
					return
				}
				if (this.selectSecurity.sex != '0' && this.selectSecurity.sex != '1') {
					this.$Message.warning('性别为必填项')
					return
				}
				if (!this.selectSecurity.id_card.trim()) {
					this.$Message.warning('身份证为必填项')
					return
				}
				if (!this.selectSecurity.security_no.trim()) {
					this.$Message.warning('保安号为必填项')
					return
				}
				if (!this.selectSecurity.tel.trim()) {
					this.$Message.warning('电话为必填项')
					return
				}
				let func = '/safety/addFullTimeSecurityTeam.do'
				if (this.idcard) {
					func = '/safety/updateFullTimeSecurityTeam.do'
				}
				axios.post(func, {
					province_code: this.selectSecurity.province_code,
					city_code: this.selectSecurity.city_code,
					town_code: this.selectSecurity.town_code,
					township_code: this.selectSecurity.township_code,
					village_code: this.selectSecurity.village_code,
					unit_id: this.selectSecurity.unit_id,
					name: this.selectSecurity.name,
					sex: this.selectSecurity.sex,
					id_card: this.selectSecurity.id_card,
					security_no: this.selectSecurity.security_no,
					tel: this.selectSecurity.tel,
					full_image: this.selectSecurity.full_image ? this.selectSecurity.full_image : '',
					image: this.selectSecurity.image,
					creator_name: this.selectSecurity.creator_name
				}).then(response => {
					if(response.data.success) {
						this.showedit = false
			...
			// 确认删除
			delsure: function () {
				axios.post('/safety/deleteFullTimeSecurityTeam.do', this.selectSecurity).then(response => {
					if(response.data.success) {
						this.showdel = false
