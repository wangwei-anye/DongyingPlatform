<template>
    <div class="security">
        <div class="main-content">
            <!-- 表格数据 -->
            <div v-if="HighlightPersonList && HighlightPersonList.length > 0">
                <div class="search-count">
                    <label>共查找到</label>&nbsp;&nbsp;&nbsp;<label style="color: white">{{searchPersonTotal}}</label>&nbsp;&nbsp;&nbsp;<label>条结果</label>
                    <!--<label style="font-size: 17px;">&nbsp;&nbsp;&nbsp;( 温馨提示：将鼠标放入列表文字内容位置，单击可进入相应详情页面 )&nbsp;&nbsp;&nbsp;</label>-->
                </div>
                <div class="table-detail" v-for="item in HighlightPersonList">
                    ...
                    <div class="detail-container">
                        <div><img :src="item.face_image || defaultPath" class="user-image"
                                  @click="showPhoto(item.face_image)" @error="showErrorDefault"></img>
                        </div>
                        <ul class="person-ul-idcode" @click="clickToUserDetail(item)">
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">身份证号码</label>
                            </li>
                            <li v-html="item.id_code"></li>
                        </ul>
                        <ul class="person-ul" @click="clickToUserDetail(item)">
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">姓名</label>
                            </li>
                            <li v-html="item.name"></li>
                        </ul>
                        <ul class="person-ul" @click="clickToUserDetail(item)">
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">性别</label>
                            </li>
                            <li v-html="item.gender"></li>
                        </ul>
                        <ul class="person-ul" @click="clickToUserDetail(item)">
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">人员类型</label>
                            </li>
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">{{ item.person_type }}</label>
                            </li>
                        </ul>
                        <ul class="person-ul" @click="clickToUserDetail(item)">
                            <li>
                                <label v-bind:class="{isBlack: item.isblack}" class="cursor-style">电话</label>
                            </li>
                            <li v-html="item.telephone"></li>
                        </ul>
                        <ul>
                            <!--<li><span class="user-info" @click="clickToUserDetail(item.user_id)">...</span></li>-->
                            <li><img src="../../assets/image/click_to_search_detail.png" class="user-info" @click="clickToUserDetail(item)"></img></li>
                        </ul>
                    </div>
                </div>
                <div class='page-container'>
                    <v-pagination :pageSize=5 :total=searchPersonTotal :pageIndex=searchPersonPageIndex
    ...
    export default {
        ...
        data() {
            return {
                showImage: false,
                imageUrl: '',
                defaultPath: require('../../assets/image/search_person.png')
            }
        },
        computed: {
            imageStyle: function () {
                if (this.HighlightPersonList.length > 1) {
                    return {
                        'bottom': this.HighlightPersonList.length * 138 + 'px'
                    }
                }
            },
            notifyStyle: function () {
                if (this.HighlightPersonList.length > 1) {
                    return {
                        'bottom': this.HighlightPersonList.length * 98 + 'px'
                    }
                }
            },
            ...mapState([
        ...
        methods: {
            ...
            changePage: function (currentPage) {
                this.updateSearchPersonPageIndex(currentPage);
