<template>
  <v-chart-box hrefto="/index/estate" boxType='polygon' iconName='village' :w="size[0]" :h="size[1]" boxTitle="小区数据">
    <div class="chartbox">
      <div class="echarts"></div>
      <div class="total">小区总数：<span>{{ data.total }}</span></div>
    </div>
  </v-chart-box>
</template>
<script>
...
export default {
  ...
  computed: {
    vData(){
      return this.data.list ? this.data : { list: [], total: 0 }
    }
  },
  methods: {
    initOption (data) {
  ...
  mounted () {
    this.$nextTick(function(){
      ...
      const data = this.initOption(this.vData)
  ...
  watch: {
    vData (val,oldVal) {
      ...
      const data = this.initOption(val)
      this.pie.setOption(data, true)
      this.animate()
    }
  }
