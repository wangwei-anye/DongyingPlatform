  ...
  <div class='header-root'>
    ...
    <div class="searchBar">
      <div class="address" v-show="!showSearchInput">
        <Select  :value="currentTownCode" @on-change="changeTown" class="v-select">
          <Option v-for="item in allTown" :value="item.town_code" :key="item.town_code">{{ item.town_name }}</Option>
        </Select>
        <Select :value="currentPoliceCode" @on-change="changePolice"  class="v-select">
          <Option v-for="item in allPolice" :value="item.police_code" :key="item.police_code">{{ item.police_name }}</Option>
        </Select>
        <!--<Select :value="currentTownShipCode" @on-change="changeTownShip" class="v-select">
          <Option v-for="item in allTownShip" :value="item.township_code" :key="item.township_code">{{ item.township_name }}</Option>
        </Select>
        <Select :value="currentVillageCode" @on-change="changeVillage"  class="v-select">
          <Option v-for="item in allVillage" :value="item.village_code" :key="item.village_code">{{ item.village_name }}</Option>
        </Select>-->
        <Select :value="currentUnitId" @on-change="changeUnit" class="v-select">
          <Option v-for="item in allUnit" :value="item.unit_id" :key="item.unit_id">{{ item.unit_name }}</Option>
        </Select>
      </div>
      <div class="search">
        <div class="search-form" v-show="showSearchInput">
          <i @click="closeSearch"></i>
          <input type="text" @keyup.enter="search" @keyup.27="closeSearch" v-model="keyWordInput" placeholder=" 姓名 / 电话 / 身份证 / 车牌号 / 区县 / 乡镇 / 社区 / 小区"/>
        </div>
        ...
        <!-- <div class="info-tip" v-show="showInfoTips" @mouseleave="hideInfoTip" @mouseover="clearHideInfoTip">
          <ul>
            ...
            <li><router-link to="/permission">权限管理<i class="right"></i></router-link></li>
...
export default {
  ...
  created() {
    ...
    getCurrentRegion() {  // 获取权限下一级的行政区域列表
      ...
      switch(true) {
        case level < 3: this.getTown(); // 国, 省, 市
          break;
        case level === 3: this.getTownShip(); // 区县
          break;
        case level === 4: this.getVillage(); // 乡
          break;
        case level === 5 || level === 6: this.getUnit(); // 村
          break;
      }
    },
...
<style scoped lang='scss'>
  .address, .search {float: left;}
  .search {
    .search-form {
      display: inline-block;
      i {display: inline-block; position: relative; top: 7px; width: 21px; height: 21px; background: url(../../assets/image/zpop-delete-icon.png) no-repeat;}
      input {
        vertical-align: middle;
        background-color: transparent !important;
        position: relative; top: 2px; border: 2px solid #147bbc; border-radius: 6px; width: 547px; height: 39px; text-indent: .1em; color: #fff; font-size: 18px; background-color: transparent; outline: none;
        ...
        ul{
          ...
          li{
            ...
            .right{
              ...
              position: relative;
              left: 6px;
              width: 18px; height: 18px; background: url(../../assets/image/header-icon-go.png.png) no-repeat;
            }
            .edit{
              cursor: pointer;
              display: inline-block;
              ...
              float: right;
              top: 8px;
              width: 18px; height: 18px; background: url(../../assets/image/header-icon-edit.png) no-repeat;
            }
            &:nth-child(1){
              color: #fff;
