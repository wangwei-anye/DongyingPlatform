<template>
    <ul>
        <span v-for="(item,index) in tabArr" @click='tab($event)'
              :class="{active:index == activeIndex && switchAble != null,circleStyle:styleType=='circleStyle',lineStyle:styleType=='lineStyle'}"
              :style="tabStyleArr[index]">{{item}}</span>
    </ul>
</template>
<script>
    import $ from 'jquery'
